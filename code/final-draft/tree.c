...
struct TreeNode *searchBST(tree t, char* x) {
  ...
  if (strcasecmp(t->data.name,x)==0)
    return t;
  else if (strcmp(t->data.name,x)<0)
...
tree removeBST(tree t, char* x) {
  ...
  p = NULL;
  tmp = t;
  while ((tmp != NULL) && (strcasecmp(t->data.name,x)!=0)) 
    if (strcasecmp(tmp->data.name,x)>0) {
      p = tmp;
      tmp = tmp->left;
    } else {
      p = tmp;
...
void breadth_first_search(tree Root){
    ...
    for(i=1;i<=c;i++)
      for(j=1;j<=arr[i].data.total;j++)
        printf("%-20s\t%lld\t\t%s\n",arr[i].data.name,arr[i].data.account[j].balance,arr[i].data.account[j].telephone);
    printf("\n");
}
...
void transfer_data(tree Root,FILE* fout){
    int i;
    Queue *queue = CreateNewQueue(Root);
    tree RunNode = Root;
    if(RunNode != NULL){
        ...
        while(!IsEmpty(queue)){
            ...
            for(i=1;i<=RunNode->data.total;i++)
              ...
              {
                fprintf(fout,"%s\n",RunNode->data.name);
                fprintf(fout,"%s\n",RunNode->data.account[i].telephone);
                fprintf(fout,"%lld\n",RunNode->data.account[i].balance);
              }
            // if(RunNode->left != NULL || RunNode->right != NULL || !IsEmpty(queue)) printf("\n");
            if(RunNode->left != NULL) enqueue(&queue,RunNode->left);
            if(RunNode->right != NULL) enqueue(&queue,RunNode->right);
