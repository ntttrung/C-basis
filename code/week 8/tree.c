...
struct TreeNode *searchBST(tree t, char* x) {
  if (isNullTree(t))
    return NULL;
  if (strcmp(t->data.eng,x)==0)
    return t;
  else if (strcmp(t->data.eng,x)<0)
...
tree insertBST(tree t, element_t x) {
  ...
  if (strcmp(t->data.eng,x.eng)==0)
    return t;
  else if (strcmp(t->data.eng,x.eng)>0) {
...
tree removeBST(tree t, element_t x) {
  ...
  p = NULL;
  tmp = t;
  while ((tmp != NULL) && (strcmp(tmp->data.eng,x.eng)!=0)) 
    if (strcmp(t->data.eng,x.eng)) {
      p = tmp;
      tmp = tmp->left;
    } else {
      p = tmp;
