...
char* treeToString(tree t) {
  ...
  if (isNullTree(t)) {
    ...
    sprintf(result, "node(%s,%s,%s)", t->data.eng, left, right);
    free(left);
    free(right);
    return result;
  }
...
struct TreeNode *searchBST(tree t, char* x) {
  if (isNullTree(t))
    return NULL;
  if(strcasecmp(t->data.eng,x)==0)
    return t;
  else if (strcasecmp(t->data.eng,x)<0)
...
tree insertBST(tree t, element_t x) {
  ...
  if (strcasecmp(t->data.eng,x.eng)==0)
    return t;
  else if (strcasecmp(t->data.eng,x.eng)>0) {
...
void breadth_first_search(tree Root){
    ...
    if(RunNode != NULL){
        ...
        while(!IsEmpty(queue)){
            ...
            strcpy(arr[count].key,RunNode->data.eng);
    ...
    for(i=1;i<=c;i++)
        printf("%-15s %s\n",arr[i].data.eng,arr[i].data.vie);
    printf("\n");
}

void extract_file(tree Root,FILE* fin){
    int i,count=1,c;
    c=countNode(Root);
    char str[20];
    sort_abc arr[c+2];
...
tree deleteNode(tree root,char* x){
    ...
    if(strcasecmp(root->data.eng,x)>0){
        root->left = deleteNode(root->left,x);
    }else if(strcasecmp(root->data.eng,x)<0){
        ...
        if(root->left == NULL){
            ...
            root->data = temp->data;
            root->right = deleteNode(root->right,temp->data.eng);
        }
    }
    return root;
}
